 ...
 * 	(optional) Finds children recursively to determine clamping box
 *
 ...
 */
export function set(source, type, buffer, move, recursive) {
	var elem = Markup.get(source)
	move = move !== false

	var children = recursive ? _getChildrenRecursively(elem) : elem.childNodes
...
/* ------------------------------------------------------------------------------------------------------------------------------- */
// PRIVATE METHODS
function _getChildrenRecursively(el, children = []) {
	for (let i = 0; i < el.childNodes.length; i++) {
		const childNode = el.childNodes[i]

		if (childNode.childNodes.length) {
			_getChildrenRecursively(childNode, children)
		} else {
